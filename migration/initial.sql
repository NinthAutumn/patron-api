...
(
    ...
    creator_id                  int8 primary key references creator
);
...
(
    ...
    creator_rank_id int4 references creator_rank,
    updated_at      timestamptz,
    created_at      timestamptz
);
create table access
...
(
    ...
    update_post          bool,
    update_setting       bool,
    update_payout_method bool,
    withdraw_balance     bool,
    create_tier          bool,
    update_tier          bool
);
...
(
    post_id    int8 primary key references post,
    word_count int8,
    char_count int8
);
...
(
    user_id             int8 references users,
    post_poll_option_id int8 references post_poll_option,
...
(
    tier_id    int8 references tier,
    benefit_id int8 references benefit,
...
(
    payment_method_id        int8 primary key references payment_method,
    last_four                int4,
...
(
    payout_method_id        int8 primary key references payout_method,
    last_four               int4,
    brand                   card_brand_type,
    expire_date             timestamptz,
...
(
    payout_method_id int8 references payout_method,
...
(
    subscription_id int8 references subscription,
    transaction_id  int8 references transaction,
...
BEGIN
    ...
    select exists(select * from creator_access where user_id = _user_id and creator_id = _creator_id) into _exist;
    if NOT _exist then
    ...
    case
        ...
        when _type = 'audio'::post_type or _type = 'video'::post_type or _type = 'image'::post_type then
            ...
            VALUES (_post ->> 'location', _post ->> 'size'::int8, _post ->> 'file_name', _post ->> 'mime_type', now(),
                    now(), _user_id, _post -> 'meta')
