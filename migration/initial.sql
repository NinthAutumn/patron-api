...
create function create_creator(_user_id int8, _name varchar, _description text,
                               _cover int8, _category_id int4, _banner int8, _creator_rank_id int4) returns RECORD as
$$
declare
    _creator_id int8;
    _access_id  int8;
BEGIN
    insert into creator(creator_rank_id, name, description, banner_image_id, cover_image_id, category_id, user_id, updated_at,
                        created_at)
    VALUES (_creator_rank_id, _name, _description, _banner, _cover, _category_id, _user_id, now(), now())
    returning id into _creator_id;

    insert into tier(title, description, cover_image_id, price, creator_id, created_at, updated_at)
    values ('公共', '投稿を表示する相手がアカウント保持者または登録者以外にもされる', 1, 0, _creator_id, now(), now());
